...
const Todo_Items = props => {
  ...
  const renderIsHighlightStar = () => {
    if (props.highlighted) {
      return <i data-testid="highlighted" className="fas fa-star" />;
    }
  ...
  const renderIsComplete = () => {
    if (props.isComplete) {
      return (
        <TodoTextDisplayDiv>
          <LineThroughSpan>{props.desc}</LineThroughSpan>
        </TodoTextDisplayDiv>
      );
    }
  ...
  return (
    <OuttestWrapper highlighted={props.highlighted}>
      <ToDoItem data-testid="todoItem">
        <Checkbox onClick={markOneTodoComplete}>
          {renderIsTodoCompleteCheckbox()}
        </Checkbox>
        {renderIsComplete()}
        <Div>
          <CustomALink onClick={handleHighLightedChange} href="#">
            {renderIsHighlightStar()}
          </CustomALink>
        </Div>
        <Div>
          <CustomALink onClick={handleIsShouldOpenTodoEditCard} href="#">
            <i className="far fa-edit" />
          </CustomALink>
        </Div>
      </ToDoItem>

      <ToDoItemMetaCondition>some condition</ToDoItemMetaCondition>
    </OuttestWrapper>
  );
};
