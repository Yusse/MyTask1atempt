...
class UpperBar extends Component {
  ...
  renderCurrentFocusTabBar = clickHandler =>
    Pages.map(pText =>
      pText === this.state.nowPage ? (
        <CurrentSelectedBlock key={pText} onClick={clickHandler}>
          {' '}
          {pText}{' '}
        </CurrentSelectedBlock>
      ) : (
        <DIVBlock key={pText} onClick={clickHandler}>
          {pText}
        </DIVBlock>
      ),
    )
  ...
  render() {
    return (
      <NavHeader>{this.renderCurrentFocusTabBar(this.onOneTabClick)}</NavHeader>
    )
  }
}
