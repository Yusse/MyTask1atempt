...
export const getMockDataAsyncly = () =>
  new Promise((res, rej) => {
    try {
      setTimeout(() => {
        res(mockData)
...
const dispatchInitTodosToStore = createAction(
  STORE.GET_INIT_TODO_DATA,
  payload => payload,
)

const dispatchReorderTodosToStore = createAction(
  STORE.REORDER_TODOS,
  payload => payload,
)
...
export default function* todosRootSaga() {
  yield all([getInitTodos(), reorderTodos()])
}
